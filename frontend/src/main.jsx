  ...
  return (
    <div className="overlay">
      <section className="modal small">
        ...
        <label>Von{local.map_date_filter_from ? <span className="field-with-button"><input type="date" disabled={saving} value={local.map_date_filter_from} onChange={(e) => setLocal((prev) => ({ ...prev, map_date_filter_from: e.target.value }))} /><button type="button" className="image-remove" style={{ minHeight: "auto", width: "auto", padding: "6px 8px" }} onClick={clearFrom} aria-label="Löschen"><Trash2 size={16} /></button></span> : <input type="date" disabled={saving} value="" onChange={(e) => setLocal((prev) => ({ ...prev, map_date_filter_from: e.target.value }))} />}</label>
        <label>Bis{local.map_date_filter_to ? <span className="field-with-button"><input type="date" disabled={saving} value={local.map_date_filter_to} onChange={(e) => setLocal((prev) => ({ ...prev, map_date_filter_to: e.target.value }))} /><button type="button" className="image-remove" style={{ minHeight: "auto", width: "auto", padding: "6px 8px" }} onClick={clearTo} aria-label="Löschen"><Trash2 size={16} /></button></span> : <input type="date" disabled={saving} value="" onChange={(e) => setLocal((prev) => ({ ...prev, map_date_filter_to: e.target.value }))} />}</label>
