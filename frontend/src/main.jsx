  ...
  return (
    <div className="overlay">
      <section className="modal small">
        ...
        <label>Von<span className="field-with-button"><input type="date" disabled={saving} value={local.map_date_filter_from || ""} onChange={(e) => setLocal((prev) => ({ ...prev, map_date_filter_from: e.target.value }))} />{local.map_date_filter_from ? <button type="button" className="image-remove" style={{ minHeight: "auto", width: "auto", padding: "6px 8px" }} onClick={clearFrom} aria-label="Löschen"><Trash2 size={16} /></button> : null}</span></label>
        <label>Bis<span className="field-with-button"><input type="date" disabled={saving} value={local.map_date_filter_to || ""} onChange={(e) => setLocal((prev) => ({ ...prev, map_date_filter_to: e.target.value }))} />{local.map_date_filter_to ? <button type="button" className="image-remove" style={{ minHeight: "auto", width: "auto", padding: "6px 8px" }} onClick={clearTo} aria-label="Löschen"><Trash2 size={16} /></button> : null}</span></label>
  ...
  return (
    <main className="list-screen">
      ...
      <div className="filters">
        ...
        {tab === "abschuesse" ? <label>Von<span className="field-with-button"><input type="date" value={filters.from} onChange={(e) => { const v = e.target.value; localStorage.setItem("jagd-date-from", v); setFilters({ ...filters, from: v }); }} />{filters.from ? <button type="button" className="image-remove" style={{ minHeight: "auto", width: "auto", padding: "6px 8px" }} onClick={clearFrom} aria-label="Löschen"><Trash2 size={16} /></button> : null}</span></label> : null}
        {tab === "abschuesse" ? <label>Bis<span className="field-with-button"><input type="date" value={filters.to} onChange={(e) => { const v = e.target.value; localStorage.setItem("jagd-date-to", v); setFilters({ ...filters, to: v }); }} />{filters.to ? <button type="button" className="image-remove" style={{ minHeight: "auto", width: "auto", padding: "6px 8px" }} onClick={clearTo} aria-label="Löschen"><Trash2 size={16} /></button> : null}</span></label> : null}
      </div>
      {tab === "abschuesse" ? (
