...
function MapEvents({ data, openCreate, originPick, setOriginPick }) {
  ...
  const snapKanzelMove = (next) => {
    const pick = originPickRef.current;
    if (pick?.mode !== "move" || pick?.type !== "kanzel") return next;
    const current = pick.target;
    if (!current) return next;
    const tapPoint = map.latLngToLayerPoint(next);
    const currentPoint = map.latLngToLayerPoint([current.lat, current.lng]);
    const snapPx = 28;
    const candidates = data.abschuesse
      .map((abschuss) => shotOrigin(abschuss, data))
      .filter((origin) => origin && Number.isFinite(origin.lat) && Number.isFinite(origin.lng))
      .map((origin) => {
        const originPoint = map.latLngToLayerPoint([origin.lat, origin.lng]);
        return { origin, tapDistance: tapPoint.distanceTo(originPoint), currentDistance: currentPoint.distanceTo(originPoint) };
      })
      .filter((candidate) => candidate.tapDistance <= snapPx && candidate.currentDistance > snapPx)
      .sort((a, b) => a.tapDistance - b.tapDistance);
    return candidates[0]?.origin || next;
  };
  const setOrigin = (next) => {
    const picked = snapKanzelMove(next);
    setOriginPickRef.current({ ...originPickRef.current, origin: { type: "point", lat: picked.lat, lng: picked.lng } });
  };
  useMapEvents({
    dblclick(e) {
  ...
  return (
    <div className="overlay">
      <section className="modal small">
        ...
        <label>Von{local.map_date_filter_from ? <span className="field-with-button"><input type="date" disabled={saving} value={local.map_date_filter_from} onChange={(e) => setLocal((prev) => ({ ...prev, map_date_filter_from: e.target.value }))} /><button type="button" className="quiet" style={{ minHeight: 0, width: 36, padding: 0, background: "transparent", color: "var(--red)" }} onClick={clearFrom} aria-label="Löschen"><Trash2 size={16} /></button></span> : <input type="date" disabled={saving} value="" onChange={(e) => setLocal((prev) => ({ ...prev, map_date_filter_from: e.target.value }))} />}</label>
        <label>Bis{local.map_date_filter_to ? <span className="field-with-button"><input type="date" disabled={saving} value={local.map_date_filter_to} onChange={(e) => setLocal((prev) => ({ ...prev, map_date_filter_to: e.target.value }))} /><button type="button" className="quiet" style={{ minHeight: 0, width: 36, padding: 0, background: "transparent", color: "var(--red)" }} onClick={clearTo} aria-label="Löschen"><Trash2 size={16} /></button></span> : <input type="date" disabled={saving} value="" onChange={(e) => setLocal((prev) => ({ ...prev, map_date_filter_to: e.target.value }))} />}</label>
...
function ObjectForm({ data, form, originPick, setOriginPick, close, load, refresh, setConfirmAction }) {
  ...
  const fillWeather = (target) => {
    ...
    autofillWeather(point, values.datum || today(), values.uhrzeit || currentTime())
      .then((next) => {
        if (!next) return;
        setValues((current) => ({
          ...current,
  ...
  const deleteItem = async () => {
    ...
    setError("");
    try {
      await api(`${path}/${itemId}`, { method: "DELETE" });
      await load();
    } catch (err) {
      setError(err.message);
    } finally {
          ...
          <>
            ...
            <div className="origin-row">
              {originLabel === "Punkt gewählt" ? (
                ...
                <button type="button" onClick={() => setOriginPick({ formId, mode: "origin", target: point, origin: null })}>Von der Karte auswählen</button>
              )}
            </div>
            <NoteField value={values.notiz} onChange={(v) => set("notiz", v)} />
          </>
      ...
      <header>
        <div className="detail-title">
          ...
          <div className="detail-heading">
            <h2>{selected.type === "kamera" ? item.typ : (item.name || item.wildart)}</h2>
  ...
  return (
    <main className="list-screen">
      ...
      <div className="filters">
        ...
        {tab === "abschuesse" ? <label>Von{filters.from ? <span className="field-with-button"><input type="date" value={filters.from} onChange={(e) => { const v = e.target.value; localStorage.setItem("jagd-list-date-from", v); setFilters({ ...filters, from: v }); }} /><button type="button" className="quiet" style={{ minHeight: 0, width: 36, padding: 0, background: "transparent", color: "var(--red)" }} onClick={clearFrom} aria-label="Löschen"><Trash2 size={16} /></button></span> : <input type="date" value="" onChange={(e) => { const v = e.target.value; localStorage.setItem("jagd-list-date-from", v); setFilters({ ...filters, from: v }); }} />}</label> : null}
        {tab === "abschuesse" ? <label>Bis{filters.to ? <span className="field-with-button"><input type="date" value={filters.to} onChange={(e) => { const v = e.target.value; localStorage.setItem("jagd-list-date-to", v); setFilters({ ...filters, to: v }); }} /><button type="button" className="quiet" style={{ minHeight: 0, width: 36, padding: 0, background: "transparent", color: "var(--red)" }} onClick={clearTo} aria-label="Löschen"><Trash2 size={16} /></button></span> : <input type="date" value="" onChange={(e) => { const v = e.target.value; localStorage.setItem("jagd-list-date-to", v); setFilters({ ...filters, to: v }); }} />}</label> : null}
      </div>
      {tab === "abschuesse" ? (
      ...
      <section className="rows">{items.map((item) => <article key={item.id} className={item.status === "archiviert" ? "is-archived" : ""}>
        <div><strong>{tab === "kameras" ? item.typ : (item.name || item.wildart)}{item.status === "archiviert" ? <em>Archiviert</em> : null}</strong><span>{rowMeta(tab, item, data)}</span></div>
