  ...
  return (
    <main className="list-screen">
      ...
      <div className="filters">
        ...
        {tab === "abschuesse" ? <label>Von<span className="field-with-button"><input type="date" value={filters.from} onChange={(e) => { const v = e.target.value; localStorage.setItem("jagd-date-from", v); setFilters({ ...filters, from: v }); }} />{filters.from ? <button type="button" className="icon-btn danger-icon" onClick={clearFrom} aria-label="Löschen"><Trash2 size={16} /></button> : null}</span></label> : null}
        {tab === "abschuesse" ? <label>Bis<span className="field-with-button"><input type="date" value={filters.to} onChange={(e) => { const v = e.target.value; localStorage.setItem("jagd-date-to", v); setFilters({ ...filters, to: v }); }} />{filters.to ? <button type="button" className="icon-btn danger-icon" onClick={clearTo} aria-label="Löschen"><Trash2 size={16} /></button> : null}</span></label> : null}
      </div>
      {tab === "abschuesse" ? (
