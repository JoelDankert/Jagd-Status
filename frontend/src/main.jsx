  ...
  return (
    <div className="overlay">
      <section className="modal small">
        ...
        <label>Von{local.map_date_filter_from ? <span className="field-with-button"><input type="date" disabled={saving} value={local.map_date_filter_from} onChange={(e) => setLocal((prev) => ({ ...prev, map_date_filter_from: e.target.value }))} /><button type="button" className="danger" style={{ minHeight: "auto", width: "auto", padding: "6px 8px", borderRadius: 4 }} onClick={clearFrom} aria-label="Löschen"><Trash2 size={16} /></button></span> : <input type="date" disabled={saving} value="" onChange={(e) => setLocal((prev) => ({ ...prev, map_date_filter_from: e.target.value }))} />}</label>
        <label>Bis{local.map_date_filter_to ? <span className="field-with-button"><input type="date" disabled={saving} value={local.map_date_filter_to} onChange={(e) => setLocal((prev) => ({ ...prev, map_date_filter_to: e.target.value }))} /><button type="button" className="danger" style={{ minHeight: "auto", width: "auto", padding: "6px 8px", borderRadius: 4 }} onClick={clearTo} aria-label="Löschen"><Trash2 size={16} /></button></span> : <input type="date" disabled={saving} value="" onChange={(e) => setLocal((prev) => ({ ...prev, map_date_filter_to: e.target.value }))} />}</label>
  ...
  return (
    <main className="list-screen">
      ...
      <div className="filters">
        ...
        {tab === "abschuesse" ? <label>Von{filters.from ? <span className="field-with-button"><input type="date" value={filters.from} onChange={(e) => { const v = e.target.value; localStorage.setItem("jagd-date-from", v); setFilters({ ...filters, from: v }); }} /><button type="button" className="danger" style={{ minHeight: "auto", width: "auto", padding: "6px 8px", borderRadius: 4 }} onClick={clearFrom} aria-label="Löschen"><Trash2 size={16} /></button></span> : <input type="date" value="" onChange={(e) => { const v = e.target.value; localStorage.setItem("jagd-date-from", v); setFilters({ ...filters, from: v }); }} />}</label> : null}
        {tab === "abschuesse" ? <label>Bis{filters.to ? <span className="field-with-button"><input type="date" value={filters.to} onChange={(e) => { const v = e.target.value; localStorage.setItem("jagd-date-to", v); setFilters({ ...filters, to: v }); }} /><button type="button" className="danger" style={{ minHeight: "auto", width: "auto", padding: "6px 8px", borderRadius: 4 }} onClick={clearTo} aria-label="Löschen"><Trash2 size={16} /></button></span> : <input type="date" value="" onChange={(e) => { const v = e.target.value; localStorage.setItem("jagd-date-to", v); setFilters({ ...filters, to: v }); }} />}</label> : null}
      </div>
      {tab === "abschuesse" ? (
